...
this.fromm = function(reportXml, browsersAndPlatformReport, testConfig) {
    //set report data based on testConfig
    report.name = testConfig.reportTitle || 'Protractor Test Execution Report';
    report.screenshotPath = testConfig.screenshotPath || './screenshots';
    report.browser = testConfig.testBrowser || 'unknown';
    ...
    //generate statistics
    var testDetails = generateSummaries(reportXml, report);
    var bpreport = editReport(suitesData, allSuites);
    //set report data based on statistics
    allSuites.totalTime = getTime(allSuites.totalTime);
    report.time = report.time.toLocaleString();
  ...
  if (testConfig.outputFilename == undefined) {      
    ...
    // var browsersAndPlatformData = fileSystem.readFileSync(browsersAndPlatformReport, 'utf8');
    // var bpdata = new xmlDocument.XmlDocument(browsersAndPlatformData);
    // bpreport.chrome.passed = bpdata.childNamed("chrome").attr.passed;
    // bpreport.chrome.failed = bpdata.childNamed("chrome").attr.failed;
    // bpreport.firefox.passed = bpdata.childNamed("firefox").attr.passed;
    // bpreport.firefox.failed = bpdata.childNamed("firefox").attr.failed;
    ...
    fileSystem.writeFileSync(
      ...
      _.template(readFile(getPath('index.tmpl')))({
        ...
        piechart: readFile(getPath('piechart.js')),
        browserPiechart: readFile(getPath('browserPiechart.js')),
        osPiechart: readFile(getPath('osPiechart.js'))
      })
  ...
  //[inserte comentario]
  var editReport = function(report, allSuites) {
    var fileSystem = require('fs');
    var a = fileSystem.readFileSync('./testresults/historic_data.xml', 'utf8');
    var DOMParser = require( 'xmldom' ).DOMParser;
    var parser = new DOMParser();
    var b = parser.parseFromString(a,"text/xml");
    var x = b.getElementsByTagName(report.browser);
    return bp;
  }
  //[inserte comentario]
